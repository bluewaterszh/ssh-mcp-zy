...
HTTP mode deliberately caps one synchronous command-tool call at 100 seconds. Tunnel transports can impose a whole-request response deadline (currently about two minutes), and progress notifications do not extend it; without this guard, a long `run-command` can be cancelled by the tunnel and surface as an opaque 502. The guard cancels the remote foreground work early enough to return an actionable `FOREGROUND_TIMEOUT` error. Use `open-session(type="background", command="...")`, then `read-session-output`, for builds/tests or other work that may run for minutes. This HTTP guard is separate from `commandTimeoutMs`: stdio and background sessions retain the normal profile timeout/lifetime rules.
