...
export const TOOL_DESCRIPTIONS: Record<string, string> = {
  ...
  'run-command': 'Execute an arbitrary shell command on the remote server. May modify the system. Destructive commands require user approval. For builds, tests, training, or other work that may take over about a minute, prefer open-session(type="background") and poll read-session-output so HTTP/tunnel response deadlines cannot cancel the job.',
  'run-batch': 'Execute up to 64 shell commands sequentially in one MCP call. Every command is independently policy-checked, quota-counted and audited. Keep the whole batch short; use a background session for work that may run for minutes.',
